...
export default function PlanPhases() {
  ...
  const [phaseData, setPhaseData] = React.useState<PhaseListResponse | null>(
    null
  );
  ...
  async function handleDeletePhase(phaseId: string) {
    ...
    try {
      ...
      if (response.status === 409) {
        ...
        try {
          deleteError = (await response.json()) as DeletePhaseErrorResponse;
        } catch {
      ...
      if (!response.ok) {
        ...
        try {
          responseError = (await response.json()) as DeletePhaseErrorResponse;
        } catch {
    ...
    return (
      <div className="kanban-page">
        <main className="kanban-page-content">
          <p className="kanban-message" role={error ? "alert" : undefined}>
  ...
  return (
    <div className="kanban-page">
      ...
      <main className="kanban-page-content management-page">
        <header className="management-page-header" tabIndex={0} ref={headerRef}>
        ...
        {error ? (
          ...
          <div className="management-sections">
            <PhaseSection
              ...
              onDeletePhase={(phaseId) => void handleDeletePhase(phaseId)}
            />
            <PhaseSection
              id="past-phases"
              title="Past"
