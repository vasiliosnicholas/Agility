  ...
  async function handleAssign() {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        try {
          responseError = (await response.json()) as UpdateTicketErrorResponse;
        } catch {
          // The fallback below handles non-JSON server errors.
        }
        setError(responseError?.message ?? "Could not assign this ticket.");
        return;
      }
  ...
  async function handleRemove(ticketId: string) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setError(responseError?.message ?? "Could not remove this ticket.");
        return;
      }
          ...
          <div className="manage-tickets">
              ...
              {phaseTickets.length === 0 ? (
                ...
                <ul className="manage-tickets-list">
                  {phaseTickets.map((ticket) => (
                    <li key={ticket._id} className="manage-tickets-item">
                      <div className="manage-tickets-item-content">
                        ...
                        <span className="manage-tickets-item-meta">
                          {ticket.assigneeId
                            ? (assigneeNames.get(ticket.assigneeId) ??
                              "Unknown")
                            : "Unassigned"}
                        </span>
                      </div>
                      <Button
                        ...
                        disabled={busy}
                        onClick={() => void handleRemove(ticket._id)}
                      >
                        Remove
                      </Button>
                    </li>
            ...
            <section className="manage-tickets-section">
              ...
              {backlogTickets.length === 0 ? (
                <p className="manage-tickets-empty">No backlog tickets.</p>
              ) : (
                <Form
                ...
                >
                  ...
                  <Button
                    ...
                    disabled={busy || !selectedTicketId || !selectedAssigneeId}
                  >
                    {busy ? "Adding Ticket…" : "Add Ticket"}
                  </Button>
                </Form>
              )}
            </section>
